...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('Support Group', url="https://t.me/KDrama_Chats_and_Discussions"),
                    InlineKeyboardButton('Owner ID', url='https://t.me/THExAkib')
                ],[
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                ...
                InlineKeyboardButton('Why I Joined', callback_data='sinfo')
            ]
        ]
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        buttons = [[
                    ...
                    InlineKeyboardButton('Earn Money', callback_data="shortlink_info"),
                    InlineKeyboardButton('Owner ID', url=CHNL_LNK)
                ],[
                    InlineKeyboardButton('Help', callback_data='help'),
                    InlineKeyboardButton('About', callback_data='about')
                ],[
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton('Bot Owner', url="https://t.me/THExAkib")
                            ],
                            [
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          ...
                          InlineKeyboardButton("Bot Owner", url="t.me/THExAkib")
                         ],[ 
                            InlineKeyboardButton('🚀 Fast Download / Watch Online 🖥️', callback_data=f'generate_stream_link:{file_id}') 
                            #Don't change anything without contacting me @LazyDeveloperr
                            ]
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            filesarr.append(msg)
        k = await client.send_message(chat_id = message.from_user.id, text=f"<b><u>❗️❗️❗️ IMPORTANT ❗️️❗️❗️</u></b>\n\nThis Movie Files/Videos will be deleted in <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this ALL Files/Videos to your Saved Messages and Start Download there</i></b>")
        ...
        if settings['is_shortlink'] and user not in PREMIUM_USER:
            ...
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
            k = await client.send_message(chat_id=message.from_user.id,text=f"<b>📕 Name : <code>{files.file_name}</code> \n\n🔗 Size : {get_size(files.file_size)}\n\n📂 File Link : {g}\n\n<i>Note: This message is deleted in 20 mins to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton('📂 Download Now 📂', url=g)
                        ], [
    ...
    if not files_:
        ...
        try:
            ...
            title = '@THExAkib  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
            ...
            k = await msg.reply("<b><u>❗️❗️❗️ IMPORTANT ❗️️❗️❗️</u></b>\n\nThis Movie File/Video will be deleted in <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>",quote=True)
            await asyncio.sleep(600)
            await msg.delete()
    ...
    files = files_[0]
    title = '@THExAkib  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    size=get_size(files.file_size)
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@THExAkib  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
...
#         return await message.reply_text("<b>Tutorial Link Not Connected\n\nYou can Connect Using /set_tutorial command</b>")
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and str(userid) not in ADMINS:
        return await message.reply_text("<b>This Command Only Works For This Group Owner/Admin\n\nTry This Command In Your Own Group, If You Are Using Me In Your Group</b>")
    else:
...
async def stop_button(bot, message):
    msg = await bot.send_message(text="**🔄 Process Stopped. Bot Is Restarting...!**", chat_id=message.chat.id)       
