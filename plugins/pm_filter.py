...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            return await message.reply_text(f"<b>Hey {message.from_user.mention}, {str(total_results)} Results are Found In My Query {search}. \n\nThis is a Group. So, you Can't Get Files From Here\n\nJoin and Search Here - https://t.me/Korean_Drama_In_Hindi_Dubbed_4u</b>")
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>ʜᴇʏ {user} 😍 ,\n\nYou Can't Get Files From Here. Request In Our <a href=https://t.me/Korean_Drama_In_Hindi_Dubbed_4u>Movie Group</a> Or CLick The Request Button Below 👇</b>",   
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 Request Here ", url=f"https://t.me/Korean_Drama_In_Hindi_Dubbed_4u")]])
    )
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            InlineKeyboardButton("Start Bot", url=f"https://t.me/{temp.U_NAME}"),
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("Send All", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("Select ᴀɢᴀɪɴ", callback_data=f"seasons#{key}")
        ])
    else:
        btn = []
        btn.insert(0, 
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    await query.message.edit_text("Make Sure That I'm Present In Your Group!!", quote=True)
                    return await query.answer(MSG_ALRT)
            else:
                await query.message.edit_text(
                    "I'm Not Connected to any Groups!\nCheck Your Connections or Connect to a Group First",
                    quote=True
                )
                return await query.answer(MSG_ALRT)
        ...
        if groupids is None:
            await query.message.edit_text(
                "There are No Active Connections!! Connect to a Group First",
            )
            return await query.answer(MSG_ALRT)
        buttons = []
    ...
    if query.data.startswith("file"):
        ...
        if not files_:
            return await query.answer('No Such Files Exist')
        files = files_[0]
        ...
        try:
            if settings['is_shortlink'] and clicked not in PREMIUM_USER:
                await query.answer(url=f"https://t.me/{temp.U_NAME}?start=sendfiles1_{key}")
                return
            else:
            ...
            await query.answer(url=f"https://t.me/{temp.U_NAME}?start=sendfiles3_{key}")
        except Exception as e:
            logger.exception(e)
            await query.answer(url=f"https://t.me/{temp.U_NAME}?start=sendfiles4_{key}")
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Join Our Backup Channel Mahn!😒", show_alert=True)
            return
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    ...
                    InlineKeyboardButton('5 Mɪɴs' if settings["auto_delete"] else '✘ Off',
                                         callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{str(grp_id)}')
                ],
                [
        ...
        await query.message.edit_text(f"<b>Your Settings Menu For {title} Has Benn Sent To Your PM</b>")
        await query.message.edit_reply_markup(reply_markup)
        if settings is not None:
            buttons = [
                [
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hey {user.mention}, Sorry Request Is Unavailable. So, Our Moderators Can't Send It.</b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hey {user.mention}, Sorry Request Is Unavailable. So, Our Moderators Can't Send It.\n\nNote: This Message is Sent To You Because You've Blocked the Bot. To Send Message To Your PM You Must Unblock The Bot.</b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
            await query.answer("You Don't Have Sufficient Rights To Do This!", show_alert=True)
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Request Group Link", url="https://t.me/Korean_Drama_In_Hindi_Dubbed_4u")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hey {user.mention}, Your Request Has been Uploaded By Our Moderator. Kindly Search in Our Group</b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hey {user.mention}, Your Request Has been Uploaded By Our Moderator. Kindly Search in Our Group\n\nNote: This Message is Sent To You Because You've Blocked the Bot. To Send Message To Your PM You Must Unblock The Bot.</b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
            await query.answer("You Don't Have Sufficient Rights To Do This!", show_alert=True)
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hey {user.mention}, Your Request Is already Available In Bot's Database. Kindly Search In Our Group</b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hey {user.mention}, Your Request Is already Available In Bot's Database. Kindly Search In Our Group\n\nNote: This Message is Sent To You Because You've Blocked the Bot. To Send Message To Your PM You Must Unblock The Bot.</b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
            await query.answer("You Don't Have Sufficient Rights To Do This!", show_alert=True)
    ...
    # don't change anything without contacting me @creatorrio
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Back", callback_data="help"),
                    InlineKeyboardButton("Contact", url="https://t.me/THExAkib")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
        ...
        if str(grp_id) != str(grpid):
            await query.message.edit("Your Active Connection Has Been Changed. Got to Connection and Change Your Active Connection.")
            return await query.answer(MSG_ALRT)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            ...
            for file in files:
                cap += f"<b>\n📁 <a href='https://t.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        if settings["button"]:
            ...
            for file in files:
                cap += f"<b>📁 <a href='https://t.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b>"
